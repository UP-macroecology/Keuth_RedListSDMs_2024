...
# Testing different niche breadths with long dispersal values with probability 0.8
...
results <- foreach(b=1:length(width), .packages = c("raster", "RangeShiftR", "dplyr", "scales", "tibble", "ggplot2", "gridExtra", "terra"), .combine = "comb",
                   .multicombine = T, .init = list(list(), list())) %dopar% {
                     ...
                     pdf(paste0(path_input, "Output_Maps/occurrences_landscape_Breadth",width[b], "longdisp0.8_Rmax3_land2.pdf"))
...
saveRDS(results, file = paste0(path_input, "Outputs/results_nichebreadths_longdisp0.8_Rmax3_land2.rds"))
...
results <- foreach(b=1:length(width), .packages = c("raster", "RangeShiftR", "dplyr", "scales", "tibble", "ggplot2", "gridExtra", "terra"), .combine = "comb",
                   .multicombine = T, .init = list(list(), list())) %dopar% {
                     ...
                     # Occurrences for individuals without long dispersal
                     pdf(paste0(path_input, "Output_Maps/occurrences_landscape_Breadth",width[b], "longdisp0.8_Rmax3_land3.pdf"))
...
saveRDS(results, file = paste0(path_input, "Outputs/results_nichebreadths_longdisp0.8_Rmax3_land3.rds"))
