...
# ------------------------------------------------------------ #
#              07a. Join different data sets                   #
# ------------------------------------------------------------ #


#-------------------------------------------------------------------------------

# Join all data sets into one for the statistical analysis
...
foreach(sim_nr=1:nrow(sims), .packages = c("data.table", "dplyr", "tidyr", "stringr")) %dopar% {
  ...
  # Load data ----------
  data_raw <- readRDS(paste0(home_folder, "analysis_data/ExtProb_Batch", BatchNum, "_Sim", rep_nr, ".rds"))
  ...
  data_long_raw <- data_long_raw[, -which(names(data_long_raw) %in% c("extProb.x", "name", "name.x"))]
  ...
  # remove double columns
  data_long <- data_long[, -which(names(data_long) %in% c("extProb.x", "name", "name.x"))]
