...
p1 <- ggplot(subset(data_simulations, data_simulations$BatchNum == 1), aes(y=pop0, x =BatchNum))+
  geom_boxplot(position = position_nudge(x = 0.1), width = 0.15, fill = "green")+
  geom_boxplot(aes(y=pop100, x = BatchNum), position = position_nudge(x = -0.1), width = 0.15, fill = "brown")+
  geom_boxplot(aes(y=occ0, x = BatchNum), position = position_nudge(x = 0.3), width = 0.15, fill = "blue")+
  geom_boxplot(aes(y=occ100, x = BatchNum), position = position_nudge(x = -0.3), width = 0.15, fill = "gold")+
  ylim(c(0,70000))+
  theme_bw()+
...
colors <- c("Population Size Year 0" = "green", "Population Size Year 100" = "brown", "Occupancy Year 0" = "blue", "Occupancy Year 100" = "gold")
...
p1 <- ggplot(data_mean[[1]], aes(x = Year, y = habitat_size))+
  ...
  geom_ribbon(aes(ymin = mean_habitat_SDM - sd_habitat_SDM, ymax = mean_habitat_SDM + sd_habitat_SDM), col = NA, alpha = 0.15, fill = "gold")+
  geom_line(aes(x = Year, y = mean_habitat_SDM), linewidth = 1.2, col = "gold")+
...
colors <- c("Real habitat size" = "gold", "Predicted habitat size (SDM)" = "#FF6A6A", "simulated Occupancy" = "blue")
